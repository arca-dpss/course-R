...
```{r, echo = FALSE, out.width="90%"}
knitr::include_graphics("img/obj_environment.png")
```
...
In generale, ogni volta che usate delle variabili *unquoted* (senza virgolette) e queste non sono dichiarate nell'ambiente, state probabilmente usando la **non-standard evaluation** e c'è una formula da qualche parte `r emo::ji("smile")` 
...
knitr::include_graphics("img/bad_question.png")
```
]
...
.pull-right[
```{r, out.width="70%", echo = FALSE}
knitr::include_graphics("img/bad_question_after.png")
```
]
